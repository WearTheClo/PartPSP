...
def main():
    parser = argparse.ArgumentParser(description='An implementation of PartPSP.')

    # basic config
    parser.add_argument('--model', type=str, default='MLP', help='model name, options: [MLP, Resnet18, ViT]')
    parser.add_argument('--shared_layers', type=int, default=1, help='number of shared layers, options: [1, 2]')
    parser.add_argument('--pretrained', action='store_false', help='load pretrained model (only for the shared parameters). Default is True.')
    ...
    parser.add_argument('--batch_size', type=int, default=100, help='batch size of the train input data for each node')
    parser.add_argument('--ext_lr', type=float, default=0.1, help='optimizer learning rate for the extractor (shared)')
    parser.add_argument('--cla_lr', type=float, default=0.1, help='optimizer learning rate for the classifier (local)')
    parser.add_argument('--noi_lr', type=float, default=0.001, help='noise rate')
    parser.add_argument('--ext_lr_decay', action='store_false', help='Set to enable learning rate decay for the extractor. Default is True.')
    parser.add_argument('--cla_lr_decay', action='store_false', help='Set to enable learning rate decay for the classifier. Default is True.')
    ...
    parser.add_argument('--C', type=float, default=0.5, help='hyperparameter C for the sensitivity estimator')
    parser.add_argument('--lamb', type=float, default=0.3, help='hyperparameter lambda for the sensitivity estimator')
